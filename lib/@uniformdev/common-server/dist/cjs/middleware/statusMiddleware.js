...
function getStatusMiddleware(uniformConfig, logger) {
    ...
    return function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var requestToken, sitecoreUrl, rs, err, app;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        logger.info('Received service status (and version) request');
                        requestToken = req.query['uniform_token'] || req.query['token'];
                        if (token && requestToken !== token) {
                            res.sendStatus(401);
                            logger.warn('Unauthorized request has been rejected');
                            return [2 /*return*/];
                        }
                        sitecoreUrl = uniformConfig.UNIFORM_API_URL + "/sitecore/service/keepalive.aspx";
                        return [4 /*yield*/, node_fetch_1.default(sitecoreUrl)];
                    case 1:
                        rs = _a.sent();
                        if (!rs.ok) {
                            err = "Sitecore does not respond at: " + sitecoreUrl;
                            res.status(503).send(err);
                            logger.warn(err);
                            return [2 /*return*/];
                        }
                        app = tryGetAppInfo(logger);
                        res.json({
                            status: 'ready',
                            version: __1.tryGetUniformVersion(logger) || 'unknown',
                            appVersion: (app === null || app === void 0 ? void 0 : app.version) || '',
                            appDescription: (app === null || app === void 0 ? void 0 : app.description) || '',
                        });
                        return [2 /*return*/];
                }
            });
