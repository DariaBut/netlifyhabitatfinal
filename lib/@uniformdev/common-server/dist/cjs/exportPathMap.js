...
exports.exportPathMap = void 0;
var fs_1 = require("fs");
var common_1 = require("@uniformdev/common");
var _1 = require(".");
var _2 = require(".");
function exportPathMap(logger) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        var config, tmp, map, pages, home, ex_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    config = _2.parseUniformServerConfig(process.env, logger);
                    tmp = config.UNIFORM_PUBLISH_TEMP_DIR;
                    if (fs_1.existsSync(tmp + '/.publish-map.json')) {
                        logger.info('Parsing .publish-map.json...');
                        map = JSON.parse(fs_1.readFileSync(tmp + '/.publish-map.json').toString());
                        fs_1.unlinkSync(tmp + '/.publish-map.json'); // to make sure it is only used for export (it is not guarantee, but 99% case)
                        return [2 /*return*/, map];
                    }
                ...
                case 3:
                    ex_1 = _a.sent();
                    logger.error('Failed to export path map', common_1.errorToJsonObject(ex_1));
                    throw ex_1;
