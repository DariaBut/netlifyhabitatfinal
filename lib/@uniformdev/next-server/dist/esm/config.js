...
export function config(_a) {
    var _this = this;
    if (_a === void 0) { _a = {}; }
    var logger = _a.logger, nextConfig = __rest(_a, ["logger"]);
    nextConfig = nextConfig || {};
    var runtime = getBoolEnv(process.env, 'UNIFORM_IS_AT_RUNTIME', false);
    logger = logger || serverLogger;
    var ver = tryGetUniformVersion(logger);
    if (runtime && ver) {
        logger.info('Uniform version: ' + ver);
    }
    var config = parseUniformServerConfig(process.env, logger, runtime);
    console.debug = config.UNIFORM_OPTIONS_DEBUG ? console.info : function () { };
    var publicRuntimeConfig = __assign(__assign({}, (nextConfig.publicRuntimeConfig || {})), parseUniformConfig(process.env, runtime));
