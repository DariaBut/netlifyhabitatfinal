...
import { serverLogger, PublishProviderWithLocalJsonDatabase, } from '@uniformdev/common-server';
...
var AzurePublishProvider = /** @class */ (function (_super) {
    ...
    function AzurePublishProvider(_a) {
        var config = _a.config, _b = _a.logger, logger = _b === void 0 ? serverLogger : _b;
        var _this = _super.call(this, getEnv(config.env, 'AZURE_FILE', 'azure.config.json'), logger) || this;
        logger.info('Initializing AzurePublishProvider');
        _this.container = getEnv(config.env, 'AZURE_CONTAINER');
        _this.connectionString = getEnv(config.env, 'AZURE_STORAGE_CONNECTION_STRING', '');
        _this.accessKey = getEnv(config.env, 'AZURE_STORAGE_ACCESS_KEY', '');
        _this.accountName = getEnv(config.env, 'AZURE_STORAGE_ACCOUNT', '');
        _this.azureTargetUrlWithSAS = getEnv(config.env, 'AZURE_TARGET_URL_WITH_SAS', '');
        _this.azureFallbackEnabled = getEnv(config.env, 'AZURE_FALLBACK_ENABLED', '1');
        var az1 = getEnv(config.env, 'AZCOPY_PATH', '');
        var az2 = getEnv(config.env, 'UNIFORM_PUBLISH_PROVIDER_AZUREBLOB_AZCOPY_PATH', '');
        if (az1 && az2) {
        ...
        _this.blobService = _this.createBlobService() || throwException('FATAL Cannot create blob service');
        return _this;
    }
        ...
        return __awaiter(this, void 0, void 0, function () {
            function walkSync(currentDirPath, callback) {
                return __awaiter(this, void 0, void 0, function () {
                    ...
                    return __generator(this, function (_b) {
                        switch (_b.label) {
                            case 0: return [4 /*yield*/, readdirSync(currentDirPath)];
                            case 1:
                                files = _b.sent();
                                promises = [];
                                _b.label = 2;
                            ...
                            case 3:
                                ...
                                filePath = join(currentDirPath, file);
                                return [4 /*yield*/, statSync(filePath)];
                            case 4:
                                stat = _b.sent();
